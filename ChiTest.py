...
file = open("Results/ChiTest", "a")
file.write("Event - Station - Band - statistic - pvalue - Null Hypothesis" + "\n")
file.close()

x = chi2.rvs(size=7201, df=2)
n_test, bins_test, _test = plt.hist(x, bins=50, histtype='bar', log=False, color='b')
for input_file in input_files:
    created_matrix = makeMatrix(input_file, input_folder)
    file = open("Results/ChiTest", "a")
    for count, row_index in enumerate(created_matrix):
        n, bins, _ = plt.hist(row_index, bins=50, histtype='bar', log=False, color='b')
        statistic, p = sp.chisquare(n, n_test)
        YN = 'reject'
        if p > statistic:
            YN = 'accept'
        file.write(
