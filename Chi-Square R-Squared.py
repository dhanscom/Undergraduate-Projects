...
# make meta analysis program (figure out what needs to be calculated)

numBins = 500


def DistroFit(df, distroType, intervalStart, intervalEnd, n, bins, numBins):
    x = np.linspace(intervalStart, intervalEnd, numBins)
    nc = 1
    try:
        coefs, coVar = sci.optimize.curve_fit(lambda t, a, b: a * chi2.pdf(b * t, df), x, n, maxfev=1000)

        fitPoints = coefs[0] * chi2.pdf(coefs[1] * x, df)
...
for input_file in input_files:
    ...
    for count, row_index in enumerate(created_matrix):
        ...
        R2 = DistroFit(2, "chi2", bins[0], bins[numBins], n, bins, numBins)
        yn90 = 'Fail'
        yn95 = 'Fail'
        yn99 = 'Fail'
        if R2 >= .90:
        ...
        file.write(
            input_file.split()[1] + " " + input_file.split()[2].split(".")[0] + " " + str(count + 1) + " R2: " + str(
                R2) + " " + yn90 + " " + yn95 + " " + yn99 + " " + "\n")
        plt.clf()
        print(count)
    file.close()
file = open("Chi2FitTestOutput", "a")
file.write("Percent Pass: " + str(countPass90 / countTotal) + " " + str(countPass95 / countTotal) + " " + str(
    countPass99 / countTotal) + "\n")
